      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          <div>
            ...
            <p className="mb-6">
              Your trusted partner for authentic Sri Lankan experiences. We craft personalized 
              journeys to showcase the best of our beautiful island.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {POPULAR_TOURS.map((tour, index) => (
                <li key={index}>
                  <Link href={tour.href} className="hover:text-white transition">
                    {tour.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-neutral-700 pt-8 mt-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p>&copy; {new Date().getFullYear()} Dear Sri Lanka. All rights reserved.</p>
            <div className="flex space-x-6 mt-4 md:mt-0">
              {FOOTER_LINKS.map((link, index) => (
                <Link key={index} href={link.href} className="hover:text-white transition">
                  {link.label}
                </Link>
              ))}
            </div>
